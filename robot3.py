...
def handle_robot_movement(X, Y, radius, serial_port):
    ...
    if radius > 0:
        ...
        if radius < 50:
            ...
            if 85 < radius <= 100 and 200 <= X <= 350 and 150 <= Y <= 350:
                print("Command: 골프_왼쪽으로_샷")
                TX_data(serial_port, 2)  # 골프_왼쪽으로_샷
                TX_data(serial_port, 33)  # 안정화자세
            elif X < 200:
                print("Command: 왼쪽옆으로20연속_골프")
                TX_data(serial_port, 15)  # 왼쪽옆으로20_골프
                time.sleep(3)
            elif X > 350:
                print("Command: 오른쪽옆으로20연속_골프")
                TX_data(serial_port, 20)  # 오른쪽옆으로20_골프
                time.sleep(3)
            elif Y < 150:
                print("Command: 전진종종걸음_골프")
                TX_data(serial_port, 10)  # 전진종종걸음_골프
